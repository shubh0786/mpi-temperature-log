...
import { ACCENT } from '../types'; // used for progress dot style only
      ...
      <div className="max-w-lg mx-auto">
        ...
        <div className="glass-strong rounded-2xl p-4 mb-4">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-bold tc-accent">Progress</span>
          ...
          <div>
            <div className="glass-strong rounded-2xl overflow-hidden">
              <div className="relative p-5 pb-3 flex items-center gap-4 bg-card-header">
                ...
                <button onClick={() => unit && setEditId(unit.id)} className="p-2 rounded-xl hover:bg-black/5 transition-all shrink-0 tc-muted hover:tc-primary" title="Edit"><Pencil size={16} /></button>
              </div>
              ...
              <div className="p-4 pt-0 flex gap-3">
                <button onClick={goPrev} disabled={safeIdx === 0} className="flex-1 flex items-center justify-center gap-2 py-3.5 rounded-xl font-bold text-sm transition-all disabled:opacity-25 tc-secondary bg-back-btn"><ArrowLeft size={17} /> Back</button>
                <button onClick={goNext} className="flex-[1.4] flex items-center justify-center gap-2 py-3.5 rounded-xl font-bold text-sm text-white transition-all btn-accent shadow-lg">
        ...
        {units.length > 0 && (
          <div className="mt-4 glass-strong rounded-2xl p-4">
            <span className="flex items-center gap-1.5 text-xs font-bold mb-2 tc-accent"><ClipboardList size={14} /> Quick View</span>
            <div className="flex flex-wrap gap-1.5">
              {units.map(u => { const t = getTemp(u.id, selectedDate); const s = getTempStatus(u, t); return (
                <span key={u.id} className={`text-xs px-2.5 py-1 rounded-full font-semibold ${s === 'warn' ? 'bg-amber-100 text-amber-700' : ''}`}
                  style={s !== 'warn' ? { background: t ? 'rgba(0,46,109,0.08)' : 'var(--pill-inactive-bg)', color: t ? ACCENT : 'var(--pill-inactive-text)' } : undefined}>
                  {u.name}: {t ? `${t}°C` : '—'}
                </span>
              ); })}
            </div>
